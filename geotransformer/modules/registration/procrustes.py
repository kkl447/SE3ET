...
def weighted_procrustes(
    ...
    with torch.amp.autocast(device_type='cuda', enabled=False):
        ...
        if weights is None:
            weights_f = torch.ones_like(src_points_f[:, :, 0])
        else:
            weights_f = weights.float()
        weights_f = torch.where(
            torch.lt(weights_f, weight_thresh), torch.zeros_like(weights_f), weights_f
        )
        weights_f = weights_f / (torch.sum(weights_f, dim=1, keepdim=True) + eps)
        weights_f = weights_f.unsqueeze(2)  # (B, N, 1)

        src_centroid = torch.sum(src_points_f * weights_f, dim=1, keepdim=True)  # (B, 1, 3)
        ref_centroid = torch.sum(ref_points_f * weights_f, dim=1, keepdim=True)  # (B, 1, 3)
        src_points_centered = src_points_f - src_centroid  # (B, N, 3)
        ref_points_centered = ref_points_f - ref_centroid  # (B, N, 3)

        H = src_points_centered.permute(0, 2, 1) @ (weights_f * ref_points_centered)
        H_cpu = H.float().cpu()
        ...
        eye = torch.eye(3, device=device, dtype=torch.float32).unsqueeze(0).repeat(
            batch_size, 1, 1
        )
