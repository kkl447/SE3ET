...
def get_so3_from_anchors_np_zyz(face_normals, gsize=3, tetra=False, octa=False):
    """
    This function is used to get the rotation matrices from the vertices of the polyhedron 
    (icosahedron, tetrahedron, or octahedron).
    :param face_normals: (na, 3) ndarray, xyz coordinates of the vertices
    :param gsize: int, the quotient factor, gsize=3 for tetrahedron, gsize=4 for octahedron
    :param tetra: bool, whether to use the tetrahedron
    :param octa: bool, whether to use the octahedron
    :return: Rs: (na*gsize, 3, 3) ndarray, rotation matrices for each vertices and each edge

    The rotation matrix follows zyz convention in http://mathworld.wolfram.com/EulerAngles.html
    R = Rz(alpha) * Ry(beta) * Rz_(gamma)
    alpha is the rotation angle around z-axis to align the vertices to x-axis
    beta is the rotation angle around y-axis.
    After alpha and beta, the origin-vertex direction is the z-axis, 
    then gamma is the rotation angle around z-axis to generate rotation matrices along the edges of each vertices.
    """
    # alpha, beta
    ...
    if tetra:
        ...
        if gsize == 3:
            ...
            raise NotImplementedError('gsize other than 3 (for faces) or 5 (for vertices) are not supported: %d'%gsize)
    return Rs
...
def icosahedron_trimesh_to_vertices(mesh_path):
    ...
    # # 60x3x3
    # Rs = get_so3_from_anchors_np(face_normals, gsize=3) # .reshape(na, gsize, 3, 3)
...
def get_relativeV_index(Rs, vs):
    ...
    # ### debug: must make sure below is satisfied
    # trace_min_ori = np.min(trace,axis=2) # bda -> da     # find correspinding original element for each rotated
    # trace_min_rot = np.min(trace,axis=1) # bda -> bd     # find corresponding rotated element for each original
    # print('trace_min_ori', trace_min_ori)
    # print('trace_min_rot', trace_min_rot)
    # assert np.allclose(trace_min_ori, 0, 1e-3,1e-3)
    # assert np.allclose(trace_min_rot, 0, 1e-3,1e-3)
...
def get_relativeR_index(Rs):
    ...
    # # ### debug: must make sure below is satisfied
    # trace_max_ori = np.max(trace,axis=0) # bda -> da     # find correspinding original element for each rotated
    # trace_max_rot = np.max(trace,axis=2) # bda -> bd     # find corresponding rotated element for each original
    # print('trace_max_ori', trace_max_ori)
    # print('trace_max_rot', trace_max_rot)
    # assert np.allclose(trace_max_ori, 1, 1e-3,1e-3)
    # assert np.allclose(trace_max_rot, 1, 1e-3,1e-3)
