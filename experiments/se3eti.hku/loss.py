...
class CoarseMatchingLoss(nn.Module):
    ...
    def forward(self, output_dict: Dict[str, torch.Tensor]) -> torch.Tensor:
        with torch.amp.autocast(device_type='cuda', enabled=False):
            ref_feats = output_dict['ref_feats_c'].float()
            src_feats = output_dict['src_feats_c'].float()
            gt_node_corr_indices = output_dict['gt_node_corr_indices']
            gt_node_corr_overlaps = output_dict['gt_node_corr_overlaps'].float()
            gt_ref_node_corr_indices = gt_node_corr_indices[:, 0]
            gt_src_node_corr_indices = gt_node_corr_indices[:, 1]

            feat_dists = torch.sqrt(pairwise_distance(ref_feats, src_feats, normalized=True))

            overlaps = torch.zeros_like(feat_dists, dtype=torch.float32)
...
class FineMatchingLoss(nn.Module):
    ...
    def forward(
        ...
        with torch.amp.autocast(device_type='cuda', enabled=False):
            ref_node_corr_knn_points = output_dict['ref_node_corr_knn_points'].float()
            src_node_corr_knn_points = output_dict['src_node_corr_knn_points'].float()
            ref_node_corr_knn_masks = output_dict['ref_node_corr_knn_masks']
            src_node_corr_knn_masks = output_dict['src_node_corr_knn_masks']
            matching_scores = output_dict['matching_scores'].float()
            transform = data_dict['transform'].float()
...
class RotationMatchingLoss(nn.Module):
    ...
    def forward(
        ...
        attn_matrix0 = output_dict['attn_matrix0'].squeeze().float() # 4*4
        # print('attn_matrix0\n', attn_matrix0.shape, '\n', attn_matrix0)
        attn_matrix1 = output_dict['attn_matrix1'].squeeze().float() # 4*4
        # print('attn_matrix1\n', attn_matrix1.shape, '\n', attn_matrix1)

        # get ground truth rotation matrix from data_dict
        # ref_points = src_points @ rotation.T + translation
        gt_T0 = data_dict['transform'].float() # (4, 4)
        gt_R0 = gt_T0[:3, :3] # (3, 3) 
        # print('gt_R0\n', gt_R0)
